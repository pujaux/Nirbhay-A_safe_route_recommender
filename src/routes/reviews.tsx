...
type Review = {
  id: number; name: string; route: string; rating: number;
  accuracy: string; text: string; date: string;
};

const SEED: Review[] = [
  { id: 1, name: "Priya S.", route: "Lajpat Nagar → Sector 18 Noida", rating: 5, accuracy: "Yes — matched my experience", text: "The DND flyway route felt very safe at night. Fully lit and CCTV everywhere. Nirbhay got it right!", date: "10 May 2026" },
  { id: 2, name: "Rahul M.", route: "Connaught Place → Sector 62", rating: 4, accuracy: "Mostly yes — minor differences", text: "Really useful app. The NH-24 warning about Akshardham at night is accurate — that stretch does feel unsafe after 10 PM.", date: "9 May 2026" },
  { id: 3, name: "Sneha K.", route: "Saket → Noida City Centre", rating: 5, accuracy: "Yes — matched my experience", text: "As a solo female traveller I found this incredibly reassuring. The metro recommendation was spot on.", date: "8 May 2026" },
  { id: 4, name: "Aisha R.", route: "Khan Market → Hauz Khas", rating: 5, accuracy: "Yes — matched my experience", text: "Walked home at 10:30 PM following the South Ex route. Markets were buzzing, felt completely at ease.", date: "6 May 2026" },
];
...
function ReviewsPage() {
  ...
  function submit() {
    ...
    setReviews([
      { id: Date.now(), ...form, rating, date: new Date().toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" }) },
      ...reviews,
    ]);
  ...
  const labelCls =
    "text-[11px] font-semibold uppercase tracking-wider text-ink-light mb-1.5 block";

  return (
    <div className="max-w-3xl mx-auto px-5 pt-10 pb-16">
      <div className="flex items-start justify-between gap-4 mb-8 animate-fadeUp">
        <div>
          <span className="text-[11px] font-semibold uppercase tracking-wider text-blush">Community</span>
          ...
          <p className="text-ink-light text-sm mt-3 max-w-md">
            How accurate were the safety scores? Share what you saw — help the next woman walk smarter.
          </p>
        </div>
        <div className="bg-gradient-warm rounded-3xl px-5 py-4 text-center border border-white/60 shadow-soft">
          ...
          <div className="mt-1 flex justify-center"><Stars count={Math.round(parseFloat(avg))} /></div>
      ...
      <div className="bg-white/85 backdrop-blur border border-border rounded-3xl p-6 mb-8 animate-fadeUp delay-1 shadow-soft">
        ...
        <div className="grid sm:grid-cols-2 gap-3 mb-3">
          <div>
            ...
            <input className={inputCls} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Priya S." />
          </div>
          <div>
            <label className={labelCls}>Route used</label>
            <input className={inputCls} value={form.route} onChange={(e) => setForm({ ...form, route: e.target.value })} placeholder="e.g. CP → Sector 18" />
          </div>
        </div>

        <div className="mb-3">
          <label className={labelCls}>Was the safety score accurate?</label>
          <select className={inputCls} value={form.accuracy} onChange={(e) => setForm({ ...form, accuracy: e.target.value })}>
        ...
        {submitted && <p className="text-sm text-sage font-semibold mb-3">✓ Thank you! Your review has been added.</p>}

        <button onClick={submit} className="w-full bg-ink text-cream rounded-xl py-3 font-semibold text-sm hover:bg-blush transition-colors shadow-soft">
          Submit review
        </button>
      </div>
      ...
      <div className="space-y-3">
        {reviews.map((r, i) => (
          <div key={r.id} className="animate-fadeUp bg-white/85 backdrop-blur border border-border rounded-3xl p-5" style={{ animationDelay: `${i * 0.06}s` }}>
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 rounded-full bg-gradient-warm flex items-center justify-center text-ink text-xs font-bold flex-shrink-0">
                {initials(r.name)}
              </div>
