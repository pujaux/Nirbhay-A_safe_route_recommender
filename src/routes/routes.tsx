...
function RoutesPage() {
    ...
    return routesData.routes
      ...
      .map((r) => {
        ...
        return {
          ...
          duration: (r.durationMins as Record<string, number>)[cat],
        };
      })
      .sort((a, b) => b.score - a.score);
  }, [hour, gender, isNight, cat]);
  ...
  return (
    <div className="max-w-3xl mx-auto px-5 pt-8 pb-16">
      ...
      <div className="bg-white/80 backdrop-blur border border-border rounded-3xl p-6 sm:p-7 mb-6 animate-fadeUp shadow-soft">
      ...
      <div className="space-y-4">
        {ranked.map((r, i) => (
          <div
            key={r.id}
            className={`animate-fadeUp bg-white/90 backdrop-blur rounded-3xl p-5 sm:p-6 transition-shadow ${i === 0 ? "border-2 border-blush shadow-soft" : "border border-border"}`}
          ...
          >
            ...
            <div className="flex justify-between items-start gap-4 mb-4">
              ...
              <div className="text-right">
                <div
                  className={`font-display text-3xl font-black leading-none ${r.score >= 80 ? "text-sage" : r.score >= 60 ? "text-amber" : "text-destructive"}`}
                >
                  {r.score}%
                </div>
                <span
                  className={`inline-block text-[10px] font-semibold mt-1 px-2 py-0.5 rounded-full ${r.score >= 80 ? "bg-sage/10 text-sage" : r.score >= 60 ? "bg-sun-soft text-amber" : "bg-destructive/10 text-destructive"}`}
                >
                  {r.safetyLabel}
                </span>
              </div>
      ...
      <div className="mt-8">
        <p className="text-xs uppercase tracking-wider font-semibold text-ink-light mb-2 flex items-center gap-1.5">
