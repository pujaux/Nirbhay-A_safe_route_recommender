  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4">
      <div className="max-w-md text-center">
        ...
        <Link to="/" className="mt-6 inline-flex rounded-full bg-blush px-5 py-2.5 text-sm font-medium text-white">
          Walk home
        </Link>
      </div>
    </div>
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Nirbhay finds the safest route home, surfaces real reports from your community, and keeps SOS one tap away." },
      { property: "og:title", content: "Nirbhay — Walk fearless" },
      { property: "og:description", content: "A safe-route recommender built for women, by everyone." },
    ...
    <QueryClientProvider client={queryClient}>
      ...
      <footer className="mt-20 border-t border-border bg-white/40 backdrop-blur">
        <div className="max-w-6xl mx-auto px-5 py-8 text-center text-xs text-ink-light">
          ...
          <p>© {new Date().getFullYear()} Nirbhay · Built for women, by everyone</p>
        </div>
      </footer>
    </QueryClientProvider>
  );
